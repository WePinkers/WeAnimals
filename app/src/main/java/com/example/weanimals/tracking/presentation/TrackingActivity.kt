package com.example.weanimals.tracking.presentation
...
import com.example.weanimals.tracking.presenter.TrackingContract
...
class TrackingActivity : AppCompatActivity(), TrackingContract.View {
    ...
    override fun showReport(report: Report) {
        ...
        binding.detailsCard.setOnClickListener { showReportDetails(report) }
    }
    ...
    private fun setupInteractions() {
        binding.updateCard.setOnClickListener { showUpdateInformation() }
    }
    ...
    private fun showReportDetails(report: Report) {
        val animal = animalName(report.animalType)
        val urgency = urgencyName(report.urgency)
        MaterialAlertDialogBuilder(this)
            .setTitle(R.string.tracking_details_title)
            .setMessage(
                getString(
                    R.string.tracking_details_message,
                    animal,
                    report.address.ifBlank { getString(R.string.location_unknown) },
                    urgency,
                    report.description.ifBlank { getString(R.string.tracking_no_description) }
                )
            )
    ...
    private fun showUpdateInformation() {
        MaterialAlertDialogBuilder(this)
            .setTitle(R.string.tracking_update_title)
            .setMessage(R.string.tracking_update_message)
            .setPositiveButton(android.R.string.ok, null)
            .show()
    }
    ...
    private fun urgencyName(urgency: String) = when (urgency) {
        Report.URGENCY_LOW -> getString(R.string.urgency_low)
        Report.URGENCY_MEDIUM -> getString(R.string.urgency_medium)
        else -> getString(R.string.urgency_high)
    }
