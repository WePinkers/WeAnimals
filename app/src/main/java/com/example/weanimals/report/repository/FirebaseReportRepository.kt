...
import com.example.weanimals.report.domain.ReportProtocol
...
class FirebaseReportRepository(
    ...
    override fun observeReport(reportId: String): Flow<Result<Report>> = callbackFlow {
        if (reportId.isBlank()) {
            ...
            .addSnapshotListener { snapshot, error ->
                when {
                    ...
                    snapshot == null || !snapshot.exists() -> {
                        ...
                        val report = snapshot.toReportOrNull()
    ...
    override suspend fun submitReport(report: NewReport): Result<Report> = runCatching {
        ...
        saveReport(report, user, protocolNumber, photoBytes)
    }
    ...
    private suspend fun saveReport(
        ...
        photoBytes?.let { data[FIELD_PHOTO_DATA] = Blob.fromBytes(it) }
    ...
    private fun DocumentSnapshot.toReportOrNull(): Report? {
        return try {
            Report(
                id = id,
                ...
                hasPhoto = get(FIELD_PHOTO_DATA) != null
